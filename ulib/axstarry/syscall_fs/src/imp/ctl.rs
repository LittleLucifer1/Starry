...
bitflags! {
    ...
    pub struct RenameFlags: u32 {
        const NONE = 0;
        // LAB3 其他可能的选项都有什么？
        const RENAME_EXCHANGE = 1;
        const RENAME_NOREPLACE = 2;
        const RENAME_WHITEOUT = 3;
    }
}
...
pub fn syscall_renameat2(
    ...
    if let Err(_) = metadata(old_path_str) {
        return Err(SyscallError::ENOENT);
    }

    // check if the destination path is existed
    if let Err(_) = metadata(new_path_str) {
        return Err(SyscallError::ENOENT);
    }

    if let Some(flags) = RenameFlags::from_bits(flags as u32) {
        if flags.contains(RenameFlags::NONE) {
            if rename(old_path_str, new_path_str).is_err() {
                return Err(SyscallError::ENOENT);
            }
        }
        if flags.contains(RenameFlags::RENAME_EXCHANGE) {
            ...
            if rename(new_path_str, old_path_str).is_err() {
                return Err(SyscallError::EPERM);
            }
        }
    }
